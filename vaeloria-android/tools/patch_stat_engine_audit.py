...
elif 'action intent must beat nouns' not in s:
    start='''        if(canonical!=null){pick(c,canonical,defaultSecondary(canonical));return;}\n        // Cross-word semantic rules: these intentionally require multiple concepts, avoiding substring collisions.'''
    replacement='''        if(canonical!=null){pick(c,canonical,defaultSecondary(canonical));return;}\n        // Cross-word semantic rules: action intent must beat nouns that merely describe the threat/object.\n        if(has(a,"staiga sureaguoti","sureaguoti","sureaguoju","reakcijos greitis")){pick(c,"Reakcijos greitis","Refleksai");return;}'''
    if start not in s: raise SystemExit('semantic priority insertion point not found')
