...
if 'String canonical=canonicalExact(a);' not in s:
    ...
    start=s.index('        String canonical=canonicalExact(a);')
    end=s.index('        // MAGIJA IR RELIKVIJOS.',start)
    block=repl.split('''    private static void chooseStats(Check c,String a){\n''',1)[1].rsplit('''        // MAGIJA IR RELIKVIJOS.''',1)[0]
