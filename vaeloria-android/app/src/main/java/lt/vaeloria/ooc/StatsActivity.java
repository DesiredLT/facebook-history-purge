...
public class StatsActivity extends Activity {
    ...
    private void build(){
        ...
        LinearLayout head=card();head.addView(t(state.characterName.toUpperCase(Locale.forLanguageTag("lt-LT"))+" · "+perfect+" / 92 ties bazine riba",17,TEXT,true));head.addView(t("Vidutinis meistriškumas virš bazinės ribos: "+avg+" / 100",13,GOLD,true));ProgressBar total=new ProgressBar(this,null,android.R.attr.progressBarStyleHorizontal);total.setMax(100);total.setProgress(avg);total.setProgressTintList(android.content.res.ColorStateList.valueOf(GOLD));total.setProgressBackgroundTintList(android.content.res.ColorStateList.valueOf(Color.rgb(34,49,57)));head.addView(total,new LinearLayout.LayoutParams(-1,dp(8)));TextView note=t("Bazė 100/100 nusako įgimtą ir ištreniruotą pajėgumą. Kilmė, archetipas ir bruožai suteikia situacinius modifikatorius, o meistriškumas virš bazinės ribos auga naudojant savybes.",11,MUT,false);note.setPadding(0,dp(8),0,0);head.addView(note);LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,-2);hp.setMargins(dp(14),0,dp(14),dp(10));root.addView(head,hp);
