...
public class StatsActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);db=new VaeloriaDb(this);state=db.loadState();values=db.getStatValues();masteries=db.getMasteries();build();}
    ...
    private void build(){
        ...
        LinearLayout head=card();head.addView(t(state.characterName.toUpperCase(Locale.forLanguageTag("lt-LT"))+" · "+perfect+" / 92 ties maksimalia riba",17,TEXT,true));head.addView(t("Laisvi savybių taškai: "+state.attributePoints+" · 1 taškas = +2",13,TEAL,true));head.addView(t("Vidutinis meistriškumas: "+avg+" / 100",13,GOLD,true));ProgressBar total=new ProgressBar(this,null,android.R.attr.progressBarStyleHorizontal);total.setMax(100);total.setProgress(avg);total.setProgressTintList(android.content.res.ColorStateList.valueOf(GOLD));total.setProgressBackgroundTintList(android.content.res.ColorStateList.valueOf(Color.rgb(34,49,57)));head.addView(total,new LinearLayout.LayoutParams(-1,dp(8)));TextView note=t("Subalansuotame režime kilmė ir archetipas nustato pradinę bazę, o kylant lygiui gaunami savybių taškai. Meistriškumas auga atskirai naudojant konkrečią savybę.",11,MUT,false);note.setPadding(0,dp(8),0,0);head.addView(note);LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,-2);hp.setMargins(dp(14),0,dp(14),dp(10));root.addView(head,hp);
    ...
    private LinearLayout statRow(String name,int value,VaeloriaDb.Mastery mastery){int v=Math.max(0,Math.min(100,value));VaeloriaDb.Mastery m=mastery==null?new VaeloriaDb.Mastery(70,0,1000):mastery;LinearLayout wrap=col();wrap.setPadding(0,dp(8),0,dp(8));LinearLayout r=row();r.setGravity(Gravity.CENTER_VERTICAL);TextView n=t(name,12,TEXT,true);r.addView(n,new LinearLayout.LayoutParams(0,-2,1));TextView val=t(v+" / 100",10,v==100?TEAL:(v>=75?GOLD:RED),true);val.setGravity(Gravity.END);r.addView(val,new LinearLayout.LayoutParams(dp(64),-2));Button add=new Button(this);add.setText("+2");add.setTextSize(10);add.setTextColor(TEXT);add.setAllCaps(false);add.setMinWidth(0);add.setMinimumWidth(0);add.setPadding(0,0,0,0);add.setBackground(round(state.attributePoints>0&&v<100?TEAL:SUR2,11,Color.rgb(49,69,80)));add.setEnabled(state.attributePoints>0&&v<100);add.setContentDescription("Padidinti savybę "+name+" dviem taškais");add.setOnClickListener(view->{db.checkpoint("prieš savybės didinimą",state);String message=db.spendAttributePoint(name,state);Toast.makeText(this,message,Toast.LENGTH_SHORT).show();state=db.loadState();values=db.getStatValues();masteries=db.getMasteries();build();});r.addView(add,new LinearLayout.LayoutParams(dp(44),dp(40)));wrap.addView(r);
        LinearLayout mr=row();mr.setGravity(Gravity.CENTER_VERTICAL);TextView ml=t("MEISTRIŠKUMAS "+m.level+"/100",9,GOLD,true);mr.addView(ml,new LinearLayout.LayoutParams(0,-2,1));TextView xp=t("PATIRTIS "+m.xp+" / "+m.nextXp,9,MUT,false);xp.setGravity(Gravity.END);mr.addView(xp,new LinearLayout.LayoutParams(dp(125),-2));wrap.addView(mr);
